...
def is_defined_type(type_: GraphQLNamedType) -> bool:
    return not is_specified_scalar_type(type_) and not is_introspection_type(
        type_
    )
...
def print_filtered_schema(
    ...
    types = list(filter(type_filter, schema.type_map.values()))

    types = [type_ for type_ in types if not is_scalar_type(type_)]
    ...
    types.sort(key=lambda t: (classes.index(type(t)), t.name))

    abstractions = ["Query", "Mutation", "Subscription"]
    abstract_types = [type_ for type_ in types if type_.name in abstractions]

    abstract_types.sort(key=lambda t: abstractions.index(t.name))
    [types.remove(type_) for type_ in abstract_types]
    types.extend(abstract_types)
...
def print_schema_definition(schema: GraphQLSchema) -> Optional[str]:
    if not is_schema_of_common_names(schema):
        return None
...
def print_enum(type_: GraphQLEnumType) -> str:
    ...
    return (
        print_description(type_) + f"enum {type_.name}" + print_block(values)
    )
...
def print_input_object(type_: GraphQLInputObjectType) -> str:
    ...
    return (
        print_description(type_) + f"input {type_.name}" + print_block(fields)
    )
...
def print_fields(type_: Union[GraphQLObjectType, GraphQLInterfaceType]) -> str:
    fields = [
        ...
        + f": {_print_field_type(field.type)}"
        + print_deprecated(field.deprecation_reason)
        for i, (name, field) in enumerate(type_.fields.items())
        if is_field_printable(field)
    ]
    return print_block(fields)


def is_field_printable(field) -> bool:
    if field.args.get("isTfInternalField"):
        return False
    return True


def _print_field_type(field_type):
    ...
    for scalar in CUSTOM_SCALARS_TO_REPLACE_WITH_STRING_IN_APPSYNC:
        field_type = field_type.replace(
            scalar.__name__, graphene.String.__name__
        )
...
def print_input_value(name: str, arg: GraphQLArgument) -> str:
    ...
    arg_decl = f"{name}: {_print_field_type(arg.type)}"
    if default_ast:
