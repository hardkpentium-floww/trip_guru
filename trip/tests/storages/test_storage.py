...
class TestStorageImplementation:
    ...
    def test_validate_admin_user_valid(self, storage):
        ...
        storage.validate_admin_user(user_id = "e9ab68e1-95c2-41bc-966d-615a9cfd175d")
    ...
    def test_book_hotel(self, storage):
        ...
        #     storage.book_hotel(hotel_id=mock_hotel.id, book_hotel_dto=book_hotel_dto)
    ...
    def test_update_booking(self, storage):
        ...
        update_booking_dto = UpdateBookingDTOFactory(checkin_date = datetime.now() + timedelta(days=3), checkout_date = datetime.now() + timedelta(days=5))
        invalid_booking_dto = UpdateBookingDTOFactory(checkin_date = datetime.now() + timedelta(days=1), checkout_date = datetime.now() + timedelta(days=2))
    ...
    def test_get_hotels(self, storage):
        ...
        mock_hotel = HotelFactory.create_batch(5, destination_id=mock_destination.id)

        hotel_dtos = storage.get_hotels(destination_id=mock_destination.id)

        hotel_dtos = sorted(hotel_dtos, key=lambda x: x.id)
        mock_hotel = sorted(mock_hotel, key=lambda x: x.id)

        assert len(hotel_dtos) == len(mock_hotel)

        for i in range(5):
            assert hotel_dtos[i].id == mock_hotel[i].id
            assert hotel_dtos[i].name == mock_hotel[i].name
            assert hotel_dtos[i].description == mock_hotel[i].description
            assert hotel_dtos[i].tariff == mock_hotel[i].tariff
            assert hotel_dtos[i].image_urls == mock_hotel[i].image_urls
            assert hotel_dtos[i].destination_id == mock_hotel[i].destination_id
